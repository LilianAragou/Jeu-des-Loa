...
public class HurleurCreuxPiece : Piece
{
    ...
    protected override void OnAllyDeath(Piece ally)
    {
        ...
        Debug.Log($"{name}: allié mort à portée → buff 3 tours");
        ApplyBuff();
    }
    ...
    private void ApplyBuff()
    {
        ...
        for (int dy = -2; dy <= 2; dy++)
        {
            ...
            Debug.Log($"{name}: buff +1 portée à {ally.name} (reste {buffDurations[ally]} tours)");
    ...
    protected override void OnTurnStart()
    {
        ...
        foreach (var ally in toExpire)
        {
            ...
            if (buffDurations[ally] <= 0)
            {
                buffDurations.Remove(ally);
                ally.TempResetRange();
                Debug.Log($"{name}: buff expiré pour {ally.name}");
            }
        }
    }
