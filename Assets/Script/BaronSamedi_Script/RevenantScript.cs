...
public class RevenantPiece : Piece
{
    ...
    private void HandleAnyCapture(Piece attacker, Piece victim)
    {
        if (attacker.isRed == isRed)
        {
            ...
            Debug.Log($"{name}: reset compteur (capture alliée)");
        }
    }
    ...
    private void OnAnyTurnEnd()
    {
        ...
        if (endingPlayer == (isRed ? 0 : 1))
        {
            ...
            Debug.Log($"{name}: fin tour équipe {(isRed ? "Rouge" : "Bleu")} → compteur = {teamTurnsWithoutCapture}");
            if (teamTurnsWithoutCapture >= 2)
            {
                Debug.Log($"{name}: explosion déclenchée !");
                Explode();
            }
        }
    }
    ...
    private void Explode()
    {
        ...
        if (bm == null)
        {
            Debug.LogError($"{name}: pas de BoardManager trouvé, explosion annulée.");
            return;
        }
        ...
        foreach (var d in dirs)
        {
            ...
            if (t == null)
            {
                Debug.Log($"{name}: tuile hors plateau en {np}");
                continue;
            }
            if (t.currentOccupant != null)
            {
                ...
                if (p != null)
                {
                    Debug.Log($"{name}: j'explose et je détruis {p.name} en {np}");
        ...
        Debug.Log($"{name}: explosion finale, je me détruis");
